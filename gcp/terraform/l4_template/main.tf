...
resource "google_compute_instance_template" "vm_template" {
  ...
  disk {
    source_image = "projects/ml-images/global/images/c0-deeplearning-common-cu124-v20250325-debian-11-py310-conda"
  ...
  metadata_startup_script = var.auto_deploy ? format("%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s",
    ...
    "cat <<'EOF_LONGCAT' > /root/models_download/longcat.sh\n${file("${path.module}/../../workflow/image_longcat/download_models.sh")}\nEOF_LONGCAT",
    ...
    "cat <<'EOF_HIDREAM_O1' > /root/models_download/hidream_o1_image.sh\n${file("${path.module}/../../workflow/hidream_o1_image/download_models.sh")}\nEOF_HIDREAM_O1",
...
resource "google_compute_firewall" "allow_https" {
  name    = "${var.template_name}-allow-https-l4"
  network = "default"
