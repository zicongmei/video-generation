...
async function upload_image(file) {
    ...
    const response = await fetch(get_api_url('/upload/image'), {
        method: 'POST',
...
async function generate() {
    ...
    const image_file = document.getElementById('image_input').files[0];
    ...
    if (!image_file && !document.getElementById('image_preview').src) {
        alert("Please select an image first!");
        return;
    }

    generate_btn.disabled = true;
    status_div.innerText = 'Uploading image...';
    video_container.innerHTML = '';

    try {
        let filename = "";
        if (image_file) {
            const upload_res = await upload_image(image_file);
            filename = upload_res.name;
        } else {
            // If no new file selected, try to extract from existing preview if possible
            // But usually ComfyUI needs the filename it already has
            alert("Please re-select the image to upload.");
        ...
        if (api_workflow["93"]) api_workflow["93"].inputs.text = prompt_text;
        if (api_workflow["89"]) api_workflow["89"].inputs.text = negative_prompt_text;
        if (api_workflow["98"]) {
            api_workflow["98"].inputs.width = width;
            api_workflow["98"].inputs.height = height;
            api_workflow["98"].inputs.length = length;
        }
        if (api_workflow["94"]) api_workflow["94"].inputs.fps = fps;
        if (api_workflow["86"]) api_workflow["86"].inputs.noise_seed = seed;
        if (api_workflow["85"]) api_workflow["85"].inputs.noise_seed = seed;
        if (api_workflow["97"]) api_workflow["97"].inputs.image = filename;
...
function save_config() {
    ...
    localStorage.setItem('wan22_i2v_config', JSON.stringify(config));
}

function load_config() {
    const saved = localStorage.getItem('wan22_i2v_config');
    if (saved) {
